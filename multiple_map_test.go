...
func TestConcurrent(t *testing.T) {
	...
	go func() {
		for i := 0; i < iterations/2; i++ {
			...
			val, _ := m.Get(String(strconv.Itoa(i)))
...
func TestJsonMarshal(t *testing.T) {
	...
	expected := "{\"a\":1,\"b\":2}"
	m := NewMMap[String, int](WithShardsNum[String, int](SHARD_COUNT))
	m.Set("a", 1)
	m.Set("b", 2)
...
func TestKeysWhenRemoving(t *testing.T) {
	...
	Num := 10
	for i := 0; i < Num; i++ {
		go func(c *ConcurrentMulMap[String, Animal], n int) {
			c.Remove(String(strconv.Itoa(n)))
		}(m, i)
	}
	keys := m.Keys()
	for _, k := range keys {
